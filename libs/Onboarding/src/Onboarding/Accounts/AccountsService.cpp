...
{
    auto response = StatusGo::Accounts::openAccounts(QString::fromUtf8(m_statusgoDataDir.c_str()).toUtf8().data());
    if(response.containsError())
    {
        qWarning() << response.error.message;
                                           ...
                                           const QString& displayName)
{
    ...
    if(StatusGo::Accounts::openAccounts(QString::fromUtf8(m_statusgoDataDir.c_str()).toUtf8().data()).containsError()) return false;
...
bool AccountsService::setKeyStoreDir(const QString& key)
{
    ...
    const auto response = StatusGo::General::initKeystore(QString::fromUtf8(m_keyStoreDir.c_str()).toUtf8().data());
...
QString AccountsService::login(MultiAccount account, const QString& password)
{
    ...
    if(StatusGo::Accounts::openAccounts(QString::fromUtf8(m_statusgoDataDir.c_str()).toUtf8().data()).containsError())
    ...
    try
    {
        ...
        auto nodeConfigJsonStr =
            templateNodeConfigJsonStr.replace("%INSTALLATIONID%", installationId)
                ...
                .replace("%DEFAULT_TORRENT_CONFIG_DATADIR%", QString::fromUtf8(DEFAULT_TORRENT_CONFIG_DATADIR.c_str()))
                .replace("%DEFAULT_TORRENT_CONFIG_TORRENTDIR%", QString::fromUtf8(DEFAULT_TORRENT_CONFIG_TORRENTDIR.c_str()))
